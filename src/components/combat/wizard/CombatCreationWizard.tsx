...
export function CombatCreationWizard({ campaignId }: CombatCreationWizardProps) {
    ...
    return monstersQuery.data.pages.flatMap((page) =>
      page.monsters.map(
        (monster): MonsterViewModel => ({
          ...
          hp: monster.data.hitPoints.average,
          ac: monster.data.armorClass,
          actions: monster.data.actions,
          traits: monster.data.traits,
          speed: monster.data.speed,
          abilityScores: monster.data.abilityScores,
        })
      )
    );
  ...
  const handleCancel = useCallback(() => {
    navigate(`/campaigns/${campaignId}/combats`);
  }, [campaignId]);
