...
/**
 * Transforms a PlayerCharacterDTO to card data with calculated fields
 */
      ...
      <CardContent className="space-y-4">
        ...
        <div className="pt-2 border-t">
          ...
          <div className="grid grid-cols-3 gap-2">
            {Object.entries(cardData.abilityScores).map(([key, value]) => (
              <div
                key={key}
                className="flex flex-col items-center rounded-lg bg-secondary/50 p-2.5 border border-border/50"
              >
        ...
        <div className="flex items-center justify-around pt-2 border-t text-xs">
          <div className="flex items-center gap-1.5">
            ...
            <span className="font-semibold">{formatModifier(cardData.initiativeModifier)}</span>
          </div>
          <div className="h-3 w-px bg-border" />
          <div className="flex items-center gap-1.5">
            <Eye className="h-3.5 w-3.5 text-muted-foreground" />
            <span className="text-muted-foreground">Perc:</span>
            <span className="font-semibold">{cardData.passivePerception}</span>
          </div>
        </div>
      </CardContent>
    </Card>
